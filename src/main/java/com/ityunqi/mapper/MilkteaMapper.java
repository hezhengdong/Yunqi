...
public interface MilkteaMapper {
    ...
    @Insert("insert into dbyunqi.milktea values(null,#{milkname},#{introduce},#{kindid},#{price},#{count},#{salenumber}) ")
    void add(Milktea milktea);

    @Delete("delete from dbyunqi.milktea where id=#{id}")
    void deletebyid(int id);

    @Update("update dbyunqi.milktea set id=#{id},milkname=#{milkname},introduce=#{introduce},kindid=#{kindid},price=#{price},count=#{count},salenumber=#{salenumber} where id=#{id}")
    void update(Milktea milktea);


    public List<Milktea> kindidselect(int kindid);

    public List<Milktea> salenumberselect(int salenumber);
}
