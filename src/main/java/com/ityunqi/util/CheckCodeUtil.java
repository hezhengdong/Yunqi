...
public class CheckCodeUtil {
    ...
    //创建了一个FileOutputStream对象，并指定了要写入的文件路径
    //声明了一个OutputStream类型的变量名为 "fos"，用于引用创建的文件输出流对象。
    //这行代码的作用是创建了一个名为 "a.jpg" 的文件输出流对象，
    // 并将其赋值给名为 "fos" 的变量，该流可以用于将数据写入到 "d://a.jpg" 文件中。

    //图片如何放到页面上
    //在main中，使用fos引用文件输出流对象，将其输出到d盘的a.jpg文件中
    //将输出流换成response的字节输出流，就可以把图片数据输出到页面上了
    //作用：防止机器自动注册，攻击服务器
